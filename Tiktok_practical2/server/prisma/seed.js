...
async function main() {
  ...
  for (let userIndex = 0; userIndex < users.length; userIndex++) {
    ...
    for (let j = 1; j <= 5; j++) {
      const videoUrl = `https://example.com/videos/user${userNumber}_video${j}.mp4`;
      const video = await prisma.video.create({
        data: {
          userId,
          title: `Video ${j} from user ${userNumber}`,
          caption: `Video ${j} from user ${userNumber}`,
          url: videoUrl,
          videoUrl,
          thumbnailUrl: `https://example.com/thumbnails/user${userNumber}_video${j}.jpg`,
          audioName: `Original Sound - User ${userNumber}`,
